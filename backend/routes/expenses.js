...
router.post('/add', async (req, res) => {
    ...
    if (!category || amount <= 0 || numberOfPayments <= 0) {
        return res.status(400).send({ message: 'Invalid input. Ensure category is specified and amount is greater than zero.' });
    }
    ...
    const expense = {
        category,
        amount,
        monthlyPayment: numberOfPaymentsAfterCasting > 1 ? amount / numberOfPaymentsAfterCasting : 0,
...
router.get('/retrieve', async (req, res) => {
    ...
    if (!year) {
        return res.status(400).send({ message: 'Year is a required query parameter.' });
    }
    year = Number(year);
    ...
    try {
        ...
        const filteredExpenses = user.expenses.filter(expense => {
            ...
            const expenseEndDate = expense.endDate ? new Date(expense.endDate) : expenseStartDate ;
